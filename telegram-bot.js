...
require('dotenv').config({ path: 'C:\\EV-System\\shared\\config\\.env' });
...
// 설정
const CONFIG = {
  token: process.env.TELEGRAM_BOT_TOKEN_EV0 || '8336098140:AAELT3_riGUGMXE3w2nDid-8FHX_vOuXzKk',
  chatId: process.env.TELEGRAM_CHAT_ID || '35391597',
  baseDir: process.env.EV_BASE_DIR || 'C:\\EV-System',
  logDir: 'C:\\EV-System\\data\\logs'
};
...
bot.onText(/\/run(?:\s+(.+))?/, function(msg, match) {
  ...
  // 실행 시작
  bot.sendMessage(msg.chat.id, '🚀 ' + botInfo.name + ' 실행 시작...\n\n⏳ 완료되면 알려드릴게요.');
  
  botInfo.status = 'running';
  var startTime = getKSTDate();
  writeLog(botKey, '실행 시작', 'info');
  
  var scriptPath = path.join(CONFIG.baseDir, botInfo.path, botInfo.script);
  var workingDir = path.join(CONFIG.baseDir, botInfo.path);
  
  // 스크립트 존재 확인
  if (!fs.existsSync(scriptPath)) {
    bot.sendMessage(msg.chat.id, '❌ 스크립트를 찾을 수 없습니다:\n' + scriptPath);
    botInfo.status = 'error';
    return;
  }
  
  var command = 'cd "' + workingDir + '" && node "' + botInfo.script + '"';
  
  var childProcess = exec(command, { 
    maxBuffer: 1024 * 1024 * 10,
    cwd: workingDir 
  }, function(error, stdout, stderr) {
    var endTime = getKSTDate();
    var elapsed = ((endTime - startTime) / 1000 / 60).toFixed(1);
    
    delete runningProcesses[botKey];
    
    if (error) {
      botInfo.status = 'error';
      botInfo.lastRun = endTime;
      writeLog(botKey, '실행 실패: ' + error.message, 'error');
      
      bot.sendMessage(msg.chat.id, 
        '❌ ' + botInfo.name + ' 실행 실패\n\n' +
        '⏱️ 소요시간: ' + elapsed + '분\n' +
        '📛 오류: ' + error.message.slice(0, 200)
      );
    } else {
      botInfo.status = 'success';
      botInfo.lastRun = endTime;
      writeLog(botKey, '실행 완료 (' + elapsed + '분)', 'success');
      
      bot.sendMessage(msg.chat.id,
        '✅ ' + botInfo.name + ' 실행 완료!\n\n' +
        '⏱️ 소요시간: ' + elapsed + '분\n' +
        '🕐 완료시각: ' + formatDateTime(endTime)
      );
    }
  ...
  runningProcesses[botKey] = childProcess;
});
...
bot.onText(/\/stop(?:\s+(.+))?/, function(msg, match) {
  ...
  if (!botKey) {
    bot.sendMessage(msg.chat.id, '❓ 중지할 봇 이름을 입력하세요.\n예: /stop 올리브영');
    return;
  }
  ...
  var process = runningProcesses[botKey];
  
  if (!process) {
    bot.sendMessage(msg.chat.id, '⚠️ ' + botInfo.name + ' 실행 중이 아닙니다.');
    return;
  }
  ...
  try {
    ...
    delete runningProcesses[botKey];
    botInfo.status = 'idle';
    writeLog(botKey, '사용자에 의해 중지됨', 'warn');
    
    bot.sendMessage(msg.chat.id, '🛑 ' + botInfo.name + ' 중지됨');
  } catch (error) {
    bot.sendMessage(msg.chat.id, '❌ 중지 실패: ' + error.message);
  }
});
